...
The Windows IoT Enterprise edition is licensed to allow you to build fixed purpose devices like Thin Clients, ATM Machines, Point-of-Sale Terminals, Industiral Automation systems, etc.  There are specific allowances and restrictions in the license agreement. In general, a fixed purpose device differs from a general purpose device in the following ways:  
* The device is locked to a single application or a fixed-set of applications via the Assigned Access or Shell Launcher feature.  
* The device powers-on immediately to the locked experience when recieved by the end-customer. This is achieved by preconfiguring the image to skip the standard Windows out-of-box experiences.
* Keyboards, USB ports, and system policies are locked down to constrain the device to be used only in its fixed purpose.  
...
>[!NOTE] See your licensing agreement for detailed guidance on all Windows IoT Enterprise usage scenarios.
...
> | Management | MDM | MDM |
> | Device Security Technologies | TPM, Secure Boot, BitLocker, Device Health Attestation, and Device Guard | TPM, Secure Boot, BitLocker, Device Guard, Defender ATP, and Device Health Attestation |
